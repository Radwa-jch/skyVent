...
// ✅ Show "Choose location" button
function showChooseLocationButton() {
  const btn = document.getElementById("chooseLocationBtn");
  if (btn) {
    btn.style.display = "inline-block";
  }
}

// ✅ Redirect to maps.html when button clicked
function goToMaps() {
  ...
  fetch(url)
    ...
    .then(data => {
      if (data.results && data.results.length > 0) {
        ...
        // ✅ Save location for maps.html
        localStorage.setItem("selectedRegion", JSON.stringify({ lat, lng }));

        // ✅ Show the maps button
        showChooseLocationButton();
      } else {
